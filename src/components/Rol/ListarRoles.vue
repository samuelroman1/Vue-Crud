		...
		<div class="card">
			...
			<div class="card_body">
						...
						<tr v-for="Rol in Roles" :key="Rol.pkRol">
							...
							<td>
								<div class="btn-group" role="label" aria-label=""> 
									...
									|<button type="button" v-on:click="borrarRol(Rol.pkRol)"
				...
				|<router-link to="CrearRoles" class="btn btn-info">CrearRol</router-link> |
							<button type="button" v-on:click="editarRol()" class="btn btn-info">
										EditarRol</button>
			</div>
		</div>
	</div>
...
export default {
	...
	methods: {
		...
		editarRol() {
			window.location.href = "/EditarRoles";
		},
		borrarRol(id) {
			var pregunta = window.confirm('Esta se seguro de eliminar este registro?');

			if (pregunta === true) {
				axios.delete("https://localhost:7051/Rol/" + id);
